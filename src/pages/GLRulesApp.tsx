...
import { Account, GLRuleSet, LANE_IDS } from '../types/gl-rules';
...
export function GLRulesApp() {
  ...
  const [activeTab, setActiveTab] = useState<TabId>(
    (searchParams.get('tab') as TabId) || 'gl-batches'
  );
  ...
  // Client filter for invoice batches (from URL deep-link)
  const [invoiceClientFilter, setInvoiceClientFilter] = useState<number | null>(
    searchParams.get('client') ? parseInt(searchParams.get('client')!, 10) : null
  );
  ...
  useEffect(() => {
    ...
    // Include client filter for invoice batches tab
    if (activeTab === 'invoices' && invoiceClientFilter) {
      params.client = invoiceClientFilter.toString();
    }
    setSearchParams(params);
  }, [activeTab, invoiceClientFilter, setSearchParams]);
  ...
  const tabs = [
    {
      id: 'gl-batches' as TabId,
      label: 'GL Batch Runs',
      icon: RefreshCw,
      description: 'Monitor GL batch processing status and trigger catchup runs',
    },
    {
      id: 'entries' as TabId,
      label: 'Accounting Entries',
      icon: FileText,
      description: 'View accounting entries generated by GL rules evaluation',
    },
    {
      id: 'invoices' as TabId,
      label: 'Invoice Batches',
      icon: Receipt,
      description: 'Create and manage invoice batches for on-account clients',
    },
    {
