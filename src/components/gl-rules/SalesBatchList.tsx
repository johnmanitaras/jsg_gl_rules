 ...
 *
 * Displays a list of sales batch runs with filtering, pagination, and expandable rows.
...
function BatchEntriesPanel({
  ...
  useEffect(() => {
    if (!hasFetched) {
      fetchBatchEntries(batchId, 'sales', 1, ENTRIES_PAGE_SIZE).then(() => {
        setHasFetched(true);
      }).catch(() => {
        setHasFetched(true);
      });
  ...
  const handleEntriesPageChange = (page: number) => {
    setEntriesPage(page);
    fetchBatchEntries(batchId, 'sales', page, ENTRIES_PAGE_SIZE);
  };
  ...
  const handleExport = async () => {
    ...
    const ok = await exportJournal(batchId, 'sales', selectedFormat);
  ...
  const handleStatusAction = async (action: string): Promise<boolean> => {
    const table = getTableName('sales');
    switch (action) {
      case 'mark-exported':
